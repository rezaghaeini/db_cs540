...
def search_movie_by_title(title):
    movies = movie_find_all({'Title': {'$regex' : title.lower(), '$options' : 'i'} })
    return movies

# done
def get_matched_person_names(name):
    actor_movies = movie_find_all({'Actors': {'$regex' : name.lower(), '$options' : 'i'} })
    writer_movies = movie_find_all({'Writer': {'$regex' : name.lower(), '$options' : 'i'} })
    director_movies = movie_find_all({'Director': {'$regex' : name.lower(), '$options' : 'i'} })
    people = {}
...
def get_tweets_contains(word_list):
    if len(word_list) > 1:
        ...
        for word in word_list:
            query.append({"text": {'$regex': word.lower(), '$options' : 'i'}})

        tweets = tweet_find_all({'$and' : query})
    else:
        tweets = tweet_find_all({'text' : {'$regex': word_list[0].lower(), '$options' : 'i'}})
    return tweets
...
# favorite_count
# retweet_count
# projection in tweets
# improve regex (not part of a word)
